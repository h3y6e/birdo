...
void SwitchController_::pressDpad(Dpad dpad) {
  _joystickInputData.Dpad = (uint8_t)dpad;
  sendReport();
}

void SwitchController_::releaseDpad() {
  _joystickInputData.Dpad = (uint8_t)Dpad::CENTER;
  sendReport();
}

void SwitchController_::setStickTiltRatio(int16_t lx, int16_t ly, int16_t rx,
                                          int16_t ry) {
  _joystickInputData.LX = (uint8_t)(lx * 0xFF / 200 + 0x80);
  _joystickInputData.LY = (uint8_t)(ly * 0xFF / 200 + 0x80);
  _joystickInputData.RX = (uint8_t)(rx * 0xFF / 200 + 0x80);
  _joystickInputData.RY = (uint8_t)(ry * 0xFF / 200 + 0x80);
  sendReport();
}
